...
def new_simulation():
    ...
    # Build reliability metrics for each station group if downtime is considered
    local_group_metrics = {}
    ...
    # Compute equipment-level reliability parameters
    equipment_params = {}
    for grp, eqs in valid_groups.items():
        rel = local_group_metrics.get(grp) if consider_downtime else None
        for eq_name in eqs:
            if rel and rel[0] is not None:
                equipment_params[eq_name] = rel
            else:
                equipment_params[eq_name] = None
    # Compute zone-level aggregated parameters for reporting (not used in simulation)
    zone_params = {}
    for i, zone in enumerate(lockout_zones):
        zone_name = f"L{i+1}"
        if consider_downtime:
            ...
            for g in zone:
                gm = local_group_metrics.get(g)
                if gm and gm[0] is not None and gm[0] > 0 and gm[1] is not None:
                    mtbf_g, mttr_g = gm
                    failure_rate += 1.0 / mtbf_g if mtbf_g > 0 else 0.0
                    # Compute availability for this equipment
                    avail_product *= (mtbf_g / (mtbf_g + mttr_g)) if (mtbf_g + mttr_g) > 0 else 1.0
                    valid_metrics_found = True
            if valid_metrics_found and failure_rate > 0:
                mtbf_zone = 1.0 / failure_rate
                # Prevent unrealistic values
                availability_zone = avail_product
                # Avoid zero or negative availability
                if availability_zone <= 0:
                    zone_params[zone_name] = None
                else:
                    mttr_zone = mtbf_zone * (1.0 / availability_zone - 1.0)
                    zone_params[zone_name] = (mtbf_zone, mttr_zone)
            else:
                zone_params[zone_name] = None
        else:
            zone_params[zone_name] = None
...
def edit_simulation():
    ...
    if st.button("▶️ Run Simulation Again"):
        try:
            ...
            equipment_params = sim_data.get("equipment_params")
            # Convert zone_params into expected format (zone_name -> (mtbf, mttr)) if stored as dict or nested dict
            ...
            if zone_params:
                ...
                for k, v in zone_params.items():
                    if v is None:
                        ...
                        # In case the params were stored as dict with mtbf/mttr
                        converted_zone_params[k] = (v.get("mtbf"), v.get("mttr"))
                    elif isinstance(v, (list, tuple)) and len(v) == 2:
                        converted_zone_params[k] = (v[0], v[1])
                    else:
                        converted_zone_params[k] = None
            # Convert equipment_params entries that may be stored as dict
            converted_equip_params = None
            if equipment_params:
                converted_equip_params = {}
                for eq, vals in equipment_params.items():
                    if vals is None:
                        converted_equip_params[eq] = None
                    elif isinstance(vals, dict):
                        converted_equip_params[eq] = (vals.get("mtbf"), vals.get("mttr"))
                    elif isinstance(vals, (list, tuple)) and len(vals) == 2:
                        converted_equip_params[eq] = (vals[0], vals[1])
                    else:
                        converted_equip_params[eq] = None
            ...
            run_result = run_simulation_backend(
                ...
                equipment_params=converted_equip_params
            )
    ...
    if st.button("💾 Save Edited Simulation"):
        try:
            ...
            # Ensure reliability parameters persist: convert any tuples to lists for JSON serialization
            # Convert zone_params and equipment_params to serializable form
            ...
            if "zone_params" in sim_data:
                sim_data["zone_params"] = serialize_params(sim_data["zone_params"])
            if "equipment_params" in sim_data:
                sim_data["equipment_params"] = serialize_params(sim_data["equipment_params"])
            with open(os.path.join(SAVE_DIR, f"{save_as}.json"), "w") as f:
...
def run_simulation_backend(station_groups_data, connections_list, from_stations_dict, duration,
                           lockout_zones=None, zone_params=None, equipment_params=None, downtime_info=None,
    ...
    # Prepare zone parameters: use provided zone_params if available (for aggregated mode),
    # or compute from downtime_info. When equipment_params is provided, zone_params are ignored.
    ...
    sim = FactorySimulation(env, station_groups, duration, dict(connections), from_stations_dict,
                            ...
                            equipment_params=equipment_params, buffer_capacities=buffer_capacities)
    env.process(sim.run())
...
def run_buffer_capacity_analysis(station_groups_data, connections_list, from_stations_dict, duration,
                                 lockout_zones, zone_params, equipment_params, capacities):
    """
    Run simulations across a range of buffer capacities for stacker/WIP cart groups to determine
    how throughput and WIP vary. A single capacity value will be applied to all stacker/WIP cart
    stations. Returns a list of dictionaries with capacity and performance metrics.

    Parameters:
        station_groups_data: list of dicts {group_name, equipment}
        connections_list: list of (src, dst) tuples
        from_stations_dict: dict group -> list of predecessors
        duration: simulation duration
        lockout_zones: list of lists defining lockout zones
        zone_params: dict zone_name -> (mtbf, mttr) used for plotting only
        equipment_params: dict eq -> (mtbf, mttr) used for reliability in simulation
        capacities: list of integer capacities to test for stacker/WIP cart groups

    Returns:
        List of dicts with keys: 'capacity', 'throughput', 'wip_metrics'
    """
    results = []
    # Identify all groups that are stackers or WIP carts
...
class FactorySimulation:
    def __init__(self, env, station_groups, duration, connections, from_stations,
                 group_to_zone=None, zone_params=None, equipment_params=None, buffer_capacities=None):
        self.env = env
        ...
        # Store equipment-level reliability parameters (eq -> (mtbf, mttr))
        self.equipment_params = equipment_params or {}
        # Store zone-level parameters (used only for reporting/plots, not for simulation when equipment_params is provided)
        self.zone_params = zone_params or {}
        # Initialize zone down counters: counts of current failures per zone
        self.zone_down_count = {z: 0 for z in set(self.group_to_zone.values())}
        ...
        if self.equipment_params:
            ...
            # Otherwise, if zone-level parameters exist (legacy aggregated mode), start zone failure processes
    ...
    def zone_failure_process(self, zone, mtbf, mttr):
        ...
        while True:
            ...
            # Zone goes down: increment the down counter
            self.zone_down_count[zone] = self.zone_down_count.get(zone, 0) + 1
    ...
    def equipment_failure_process(self, eq, mtbf, mttr):
        """
        Process that generates failures for a given equipment based on exponential MTBF and erlang MTTR.
        When a failure occurs, the entire zone of the equipment is marked down until repair.
        """
        # Determine which zone this equipment belongs to
        group = self.equipment_to_group.get(eq)
        ...
        while True:
            ...
            try:
                ...
                ttf = float('inf')
            yield self.env.timeout(ttf)
            # On failure, increment the zone down counter
            self.zone_down_count[zone_name] += 1
            # Repair time using Erlang distribution (shape=2, scale=mttr/2)
            repair_time = 0.0
            if mttr and mttr > 0:
                repair_time = random.gammavariate(2, mttr / 2.0)
            yield self.env.timeout(repair_time)
            # Repair complete: decrement the zone down counter
            # Ensure it doesn't go negative
            self.zone_down_count[zone_name] = max(self.zone_down_count[zone_name] - 1, 0)
...
def show_detailed_summary(sim, valid_groups, from_stations, duration):
    ...
    # === WIP Profile for Stacker & WIP Cart Stations ===
    # This section plots WIP over time for stations whose names include 'STACKER' or 'WIP CART'
    special_groups = [g for g in groups if ("STACKER" in g.upper()) or ("WIP CART" in g.upper()) or ("WIPCART" in g.upper())]
    if special_groups:
        st.subheader("📈 WIP Over Time for Stacker & WIP Cart Stations")
        # Prepare a summary of max WIP for these groups
        max_wip_data = []
        # Determine ten equal sample points based on simulation time
        # Use the time_points recorded in the simulation
        time_points = getattr(sim, 'time_points', [])
        # Determine indices for ten equally spaced samples
        sample_indices = []
        if time_points:
            num_samples = 10
            total_points = len(time_points)
            # If fewer than num_samples points, sample all available points
            if total_points <= num_samples:
                sample_indices = list(range(total_points))
            else:
                # Compute indices at equal intervals
                for i in range(num_samples):
                    idx = int(round(i * (total_points - 1) / (num_samples - 1)))
                    sample_indices.append(idx)
        for g in special_groups:
            wip_series = sim.wip_over_time.get(g, [])
            if not wip_series:
                continue
            max_wip = max(wip_series)
            max_wip_data.append({"Station": g, "Max WIP": int(max_wip)})
            # Prepare sampled data for plotting
            if sample_indices:
                sampled_times = [time_points[i] for i in sample_indices]
                sampled_wip = [wip_series[i] for i in sample_indices]
                fig_wip, ax_wip = plt.subplots(figsize=(6, 3))
                ax_wip.plot(sampled_times, sampled_wip, marker='o')
                ax_wip.set_title(f"WIP Over Time: {g}")
                ax_wip.set_xlabel("Time (s)")
                ax_wip.set_ylabel("WIP Level")
                ax_wip.grid(True, linestyle='--', alpha=0.6)
                st.pyplot(fig_wip)
        # Display table of maximum WIP for these stations
        if max_wip_data:
            st.markdown("#### Maximum WIP at Stacker/WIP Cart Stations")
            df_max_wip = pd.DataFrame(max_wip_data)
            st.dataframe(df_max_wip)
